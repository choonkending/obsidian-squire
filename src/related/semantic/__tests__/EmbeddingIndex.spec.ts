...
describe("EmbeddingIndex", () => {
    ...
    it("allSimilarities returns empty map for empty index", () => {
        const idx = new EmbeddingIndex(makeAdapter());
        expect(idx.allSimilarities([1, 0]).size).toBe(0);
    });

    it("allSimilarities returns cosine similarity for indexed paths", () => {
        const idx = new EmbeddingIndex(makeAdapter());
        idx.set("a.md", [1, 0, 0]);
        expect(idx.allSimilarities([1, 0, 0]).get("a.md")).toBeCloseTo(1, 6);
        expect(idx.allSimilarities([0, 1, 0]).get("a.md")).toBeCloseTo(0, 6);
    });
...
describe("EmbeddingIndex persistence", () => {
    ...
    it("clears modelId when saving empty index", async () => {
        const adapter = makeAdapter();
        const idx = new EmbeddingIndex(adapter);
        await idx.load("m");
        await idx.save();
